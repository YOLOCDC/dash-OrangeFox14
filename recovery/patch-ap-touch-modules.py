#!/usr/bin/env python3
"""Create the Recovery-only Goodix AP fallback module chain from OTA inputs."""
...
COPY_SPECS = {
    "xiaomi_touch_dash.ko": (XIAOMI_TOUCH_SHA256, 211696),
}
...
RECOVERY_DEPENDENCIES = (
    "goodix_core_dash.ko: tui-common.ko xiaomi_touch_dash.ko scp.ko mtk_tinysys_ipi.ko mtk_rpmsg_mbox.ko mtk-mbox.ko",
...
def build_modules_dep(source_dir, output_dir):
    ...
    for name in ("scp.ko", "goodix_core_dash.ko", "xiaomi_touch_dash.ko"):
...
def validate_published(output_dir, expected_modules_dep):
    ...
    xiaomi = read_regular(output_dir / "xiaomi_touch_dash.ko")
    require(digest(xiaomi) == XIAOMI_TOUCH_SHA256, "published hash mismatch for xiaomi_touch_dash.ko")
...
def publish_staged(staging_dir, output_dir):
    ...
    backup_dir = Path(tempfile.mkdtemp(prefix=".dash-ap-touch-backup.", dir=output_dir.parent))
    published = []
    backed_up = []
    try:
...
def main():
    ...
    staging_dir = Path(tempfile.mkdtemp(prefix=".dash-ap-touch-stage.", dir=output_dir.parent))
    try:
        for name, spec in MODULE_SPECS.items():
    ...
    print(f"xiaomi_touch_dash.ko: {XIAOMI_TOUCH_SHA256}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"dash AP touch module generator: {error}", file=sys.stderr)
        raise SystemExit(2)
